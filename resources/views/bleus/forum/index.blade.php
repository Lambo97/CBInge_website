    ...
    <form method="POST" action="/bleus/forum/add" enctype="multipart/form-data">
        ...
        <div class="form-group row">
            ...
            <div class="textarea w-75 pl-3">
                ...
                <input type="file" class="custom-file-input buttons-blue-dark" id="photo" name="photo">
        ...
        <div class="row mx-0" @if($post->ancre == 1) style="border:2px white solid;" @endif>
            ...
            <div class="col-10">
                <div class="row mb-2 mt-2 justify-content-between">
                    <div class="col-6">
                        <h4 style='display:inline' class="mr-3"><a href="/profile/show/{{$post->auteur->id}}" class="blue-link">{{$post->auteur->surnom_forum}}</a></h4>  @if($post->ancre == 1) <small><i>Message encré</i></small> @endif
                    ...
                    <div class="col-2 col-xs-3 d-flex justify-content-end">
                        <div class="blue-link mr-2"><i class="far fa-trash-alt" data-toggle="modal" data-target="#deletePost{{$post->id}}"></i></div>
                        <a href="/bleus/forum/edit/{{$post->id}}" class="blue-link mr-2"><i class="far fa-edit"></i></a>
                        @if(Auth::user()->droit < 3)
                        <a href="/bleus/forum/ancre/{{$post->id}}" class="blue-link"><i class="fas fa-anchor"></i></a>
                        @endif
                    </div>
                    @endif
                ...
                <div class="row" id="post{{$post->id}}">
                    <div class="col-4 col-sm-2 d-flex justify-content-center">
                        @if($post->like->where('value', 1)->where('user_id', Auth::user()->id)->count() > 0)
                            <b><a href="/bleus/forum/like/{{$post->id}}" onclick="like(this, true); return false;" class="blue-link" id={{$post->id}}><i>j'aime ({{$post->like->where('value', 1)->count()}})</i></a></b>
                        @else
                            <small><a href="/bleus/forum/like/{{$post->id}}" onclick="like(this, true); return false;" class="blue-link" id={{$post->id}}><i>j'aime ({{$post->like->where('value', 1)->count()}})</i></a></small>
                        @endif
                    </div>
                    <div class="col-4 col-sm-3 d-flex justify-content-center">
                        @if($post->like->where('value', -1)->where('user_id', Auth::user()->id)->count() > 0)
                            <b><a href="/bleus/forum/dislike/{{$post->id}}"  onclick="dislike(this, true); return false;" class="blue-link" id={{$post->id}}><i>je n'aime pas ({{$post->like->where('value', -1)->count()}})</i></a></b>
                        @else
                            <small><a href="/bleus/forum/dislike/{{$post->id}}"   onclick="dislike(this, true); return false;" class="blue-link" id={{$post->id}}><i>je n'aime pas ({{$post->like->where('value', -1)->count()}})</i></a></small>
                        @endif
                    </div>
                    <div class="col-4 col-sm-2 d-flex justify-content-center">
                        <div class="blue-link mr-2" data-toggle="modal" data-target="#viewlike{{$post->id}}"><i>Consulter</i></div>
                    </div>
                </div>
                <hr>
                    ...
                    <div class="row mb-3">
                        ...
                        @if(Auth::user()->id == $comment->id_auteur || Auth::user()->droit < 3)
                            <div class="d-flex justify-content-end position-absolute right-0 mt-1">
                                <div class="blue-link mr-2"><i class="far fa-trash-alt" data-toggle="modal" data-target="#deleteComment{{$comment->id}}"></i></div>
                                <a href="/bleus/forum/comment/edit/{{$comment->id}}" class="blue-link mr-2"><i class="far fa-edit"></i></a>
                            </div>
                            @endif

                                <a href="/profile/show/{{$comment->auteur->id}}" class="blue-link mr-3">{{$comment->auteur->surnom_forum}}</a>
                            ...
                            <div class="row align-self-end" id="comment{{$comment->id}}">
                                ...
                                @if($comment->like->where('value', 1)->where('user_id', Auth::user()->id)->count() > 0)
                                    <b><a href="/bleus/forum/comment/like/{{$comment->id}}" onclick="like(this, false); return false;" class="blue-link mx-1" id={{$comment->id}}><i>j'aime ({{$comment->like->where('value', 1)->count()}})</i></a></b>
                                @else
                                    <small><a href="/bleus/forum/comment/like/{{$comment->id}}" onclick="like(this, false); return false;" class="blue-link mx-1" id={{$comment->id}}><i>j'aime ({{$comment->like->where('value', 1)->count()}})</i></a></small>
                                @endif
                                </div>
                                 <div>
                                @if($comment->like->where('value', -1)->where('user_id', Auth::user()->id)->count() > 0)
                                    <b class="mx-3"><a href="/bleus/forum/comment/dislike/{{$comment->id}}"  onclick="like(this, false); return false;"class="blue-link mx-1" id={{$comment->id}}><i>je n'aime pas ({{$comment->like->where('value', -1)->count()}})</i></a></b>
                                @else
                                    <small class="mx-3"><a href="/bleus/forum/comment/dislike/{{$comment->id}}" onclick="like(this, false); return false;" class="blue-link mx-1" id={{$comment->id}}><i>je n'aime pas ({{$comment->like->where('value', -1)->count()}})</i></a></small>
                                @endif
                                </div>
                                <div><small><a href="#" class="blue-link mx-2" data-toggle="modal" data-target="#viewlikeComment{{$comment->id}}"><i>Consulter</i></a></small></div>
                            ...
                            <div class="modal-content" style="background-color: #353131">
                                ...
                                <div class="modal-body">
                                    ...
                                    @foreach ($comment->like->where('value', 1) as $like)
                                        <a href="/profile/show/{{$like->user->id}}" class="blue-link">{{$like->user->surnom_forum}}</a> -
                                    @endforeach
                                    </div>
                                    <h6>Je n'aime pas</h6>
                                    @foreach ($comment->like->where('value', -1) as $like)
