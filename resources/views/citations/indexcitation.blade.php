...
@section('title')
<title> CB Ingé - Citations </title>
@endsection

@section('content')

<h1 class="mb-3">Les citations</h1>

<div class="row">
    <div class="col-12  form-content mb-5">
      <p class="font-weight-bold mt-3 mb-3">Ajouter une citation</p>  
    <form method="POST" action="/citations/add" enctype="multipart/form-data">
        @csrf
            ...
            <div class="col-10">
                <div class="row mb-2 mt-2 justify-content-between">
                    ...
                    <div class="col-2 col-xs-3 d-flex justify-content-end">
                        ...
                        <a href="/citations/edit/{{$post->id}}" class="green-link mr-2"><i class="far fa-edit"></i></a>
                ...
                <div class="row" id="post{{$post->id}}">
                    <div class="col-4 col-sm-2 d-flex justify-content-center">
                        @if($post->like->where('value', 1)->where('user_id', Auth::user()->id)->count() > 0)
                            <b><a href="/citations/like/{{$post->id}}" onclick="like(this, true); return false;" class="green-link" id={{$post->id}}><i>j'aime ({{$post->like->where('value', 1)->count()}})</i></a></b> 
                        @else
                            <small><a href="/citations/like/{{$post->id}}" onclick="like(this, true); return false;" class="green-link" id={{$post->id}}><i>j'aime ({{$post->like->where('value', 1)->count()}})</i></a></small>
                        @endif
                    </div>
                    <div class="col-4 col-sm-3 d-flex justify-content-center">
                        @if($post->like->where('value', -1)->where('user_id', Auth::user()->id)->count() > 0)
                            <b><a href="/citations/dislike/{{$post->id}}"  onclick="dislike(this, true); return false;" class="green-link" id={{$post->id}}><i>je n'aime pas ({{$post->like->where('value', -1)->count()}})</i></a></b>
                        @else
                            <small><a href="/citations/dislike/{{$post->id}}"   onclick="dislike(this, true); return false;" class="green-link" id={{$post->id}}><i>je n'aime pas ({{$post->like->where('value', -1)->count()}})</i></a></small>
                        @endif
                    </div>
                    <div class="col-4 col-sm-2 d-flex justify-content-center">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body text-dark">
                    Voulez-vous vraiment supprimer cette citation ?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Retour</button>
                    <a href="/citations/destroy/{{$post->id}}" class="btn btn-danger">Supprimer</a>
                </div>
            </div>
        </div>
    </div>
